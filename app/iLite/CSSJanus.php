<?php
/**
 * PHP port of CSSFlip. https://github.com/cssjanus/php-cssjanus
 *
...
/**
 * CSSFlip is a a utility that converts CSS stylesheets
...
class CSSFlip {
	...
	public static function transform($css, $options = array(), $transformEdgeInUrl = false) {
		...
		$noFlipSingle = new CSSFlipTokenizer(self::$patterns['noflip_single'], '`NOFLIP_SINGLE`');
		$css = $noFlipSingle->tokenize($css);

		// Tokenize class rules with /* @noflip */
		$noFlipClass = new CSSFlipTokenizer(self::$patterns['noflip_class'], '`NOFLIP_CLASS`');
		$css = $noFlipClass->tokenize($css);

		// Tokenize comments
		$comments = new CSSFlipTokenizer(self::$patterns['comment'], '`C`');
...
/**
 * Utility class used by CSSFlip that tokenizes and untokenizes things we want
 * to protect from being janused.
 */
class CSSFlipTokenizer {
	private $regex;
	private $token;
	private $originals;
