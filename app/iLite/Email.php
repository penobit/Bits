...
class Email extends PHPMailer{
    ...
    private static function getTemplate() {   
        if (!self::$template) {
            $html = file_get_contents(APP_PATHH."/inc/email-template.inc.php");
    ...
    private static function sendNewPaymentNotification($data = []) {
        ...
        $emailbody = "<p>Hello, </p>"
                   . "<p>New payment recevied in <a href='".APP_URL."'>".htmlchars($siteSettings->get("data.site_name"))."</a> with following data:</p>"
                   . "<div style='margin-top: 30px; font-size: 14px; color: #9b9b9b'>"
    ...
    private static function sendPasswordRecoveryEmail($data = []) {
        ...
        $emailbody = "<p>".__("Hi %s", htmlchars($user->get("firstname"))).", </p>"
                   . "<p>".__("Someone requested password reset instructions for your account on %s. If this was you, click the button below to set new password for your account. Otherwise you can forget about this email. Your account is still safe.", "<a href='".APP_URL."'>".htmlchars($siteSettings->get("data.site_name"))."</a>")."</p>"
                   . "<div style='margin-top: 30px; font-size: 14px; color: #9b9b9b'>"
                   . "<a style='display: inline-block; background-color: #3b7cff; color: #fff; font-size: 14px; line-height: 24px; text-decoration: none; padding: 6px 12px; border-radius: 4px;' href='".APP_URL."/recovery/".$user->get("id").".".$hash."'>".__("Reset Password")."</a>"
                   . "</div>";
